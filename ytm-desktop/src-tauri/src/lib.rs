...
async fn toggle_pip(app: AppHandle, url: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    if *pip_open {
        if let Some(pip) = app.get_webview_window("pip") { let _ = pip.close(); }
        *pip_open = false;
...
fn setup_tray(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _tray = TrayIconBuilder::new().icon(img).menu(&menu).tooltip("YouTube Music")
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .manage(AppState {
            audio_only: Mutex::new(false), ad_block_enabled: Mutex::new(true),
            background_playback: Mutex::new(true), pip_open: Mutex::new(false),
        })
        .invoke_handler(tauri::generate_handler![
            toggle_audio_only, get_audio_only, toggle_ad_block, get_ad_block,
            toggle_background_playback, get_background_playback,
            download_track, set_media_info, toggle_pip,
        ])
        .setup(|app| {
