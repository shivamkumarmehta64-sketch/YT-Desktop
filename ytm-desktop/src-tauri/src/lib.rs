...
fn ensure_webview2_loader() {
    ...
    if !dll.exists() { let _ = std::fs::write(&dll, WEBVIEW2_LOADER); }
}
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ad_block_enabled: true, audio_only: false, background_playback: true,
            mini_player: false, sleep_timer_minutes: 0, first_run: true,
        }
    }
}
...
fn save_settings(s: &Settings) {
    if let Ok(json) = serde_json::to_string_pretty(s) { let _ = std::fs::write(settings_path(), &json); }
}

struct AppState { settings: Mutex<Settings>, pip_open: Mutex<bool> }
...
async fn toggle_pip(app: AppHandle, url: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let pip_url = if url.is_empty() { "https://music.youtube.com" } else { &url };
    ...
    let _pip = WebviewWindowBuilder::new(&app, "pip",
        ...
        .title("YouTube Music PiP").inner_size(420.0, 260.0).min_inner_size(320.0, 200.0)
...
fn setup_tray(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _tray = TrayIconBuilder::new().icon(img).menu(&menu).tooltip("YouTube Music Desktop")
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let window = WebviewWindowBuilder::new(app, "main",
                ...
                .title("YouTube Music").inner_size(1280.0, 800.0).min_inner_size(900.0, 600.0)
                .center()
                .user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36")
            ...
            window.on_window_event(move |event| {
                if let tauri::WindowEvent::CloseRequested { api, .. } = event { api.prevent_close(); let _ = w.hide(); }
            });
            Ok(())
        })
