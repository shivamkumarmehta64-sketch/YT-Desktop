(function () {
  ...
  // Block fetch to ad domains only (no body tampering)
  ...
  // Remove ad elements – targeted selectors only, no aggressive cleanup
  function cleanAds() {
    document.querySelectorAll(
      ...
      'ytd-mealbar-promo-renderer, .ytd-display-ad-renderer, .ytd-video-masthead-ad-v3-renderer, ' +
      '.ad-showing, .ytp-ad-progress-list, .video-ads, .ytp-ad-player-overlay'
    ).forEach(el => el.remove());
  }

  // Mute + fast-forward if ad is playing
  function skipAd() {
    const v = document.querySelector('video');
    if (!v) return;
    if (document.querySelector('.ad-showing, .ytp-ad-module, .ytp-ad-image-overlay, .video-ads')) {
      v.muted = true;
  ...
  // Observe player for ad state changes
  ...
  // Sponsor skip (via chapter analysis)
  ...
  setTimeout(() => {
    ...
    </style><div id="ytp-btn">✦</div><div id="ytp-menu"><h3>YT Settings</h3></div>`;
